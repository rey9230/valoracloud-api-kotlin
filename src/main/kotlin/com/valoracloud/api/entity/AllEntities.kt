...
@Entity @Table(name = "refresh_tokens", indexes = [Index(columnList = "user_id"), Index(columnList = "token")])
class RefreshToken(
    @Id var id: String = cuid(),
...
// Plan
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "plans", indexes = [Index(columnList = "productType")])
class Plan(
    @Id var id: String = cuid(),
    @Column(nullable = false) var name: String = "",
    @Column(nullable = false, unique = true) var slug: String = "",
    @Enumerated(EnumType.STRING) @Column(nullable = false) var productType: ProductType = ProductType.CLOUD_VPS,
    var description: String? = null,
    var cpu: Int = 1, var ram: Int = 1, var disk: Int = 25,
    var diskType: String = "NVMe",
    var bandwidth: Int = 1, var portSpeed: Int? = null, var snapshots: Int = 0,
...
// Order
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "orders", indexes = [Index(columnList = "user_id"), Index(columnList = "status")])
class Order(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
    ...
    var region: String = "EU", var os: String = "ubuntu-24.04",
...
// Server
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "servers", indexes = [Index(columnList = "user_id"), Index(columnList = "status")])
class Server(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
    ...
    var os: String = "", var region: String = "EU",
    @JdbcTypeCode(SqlTypes.JSON) var contaboData: Map<String, Any>? = null,
    var provisionedAt: Instant? = null, var expiresAt: Instant? = null,
) : BaseEntity()

@Entity @Table(name = "provisioning_logs", indexes = [Index(columnList = "server_id")])
class ProvisioningLog(
    @Id var id: String = cuid(),
    @Column(name = "server_id", nullable = false) var serverId: String = "",
    var step: String = "", var status: String = "",
...
// Invoice
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "invoices", indexes = [Index(columnList = "user_id"), Index(columnList = "order_id")])
class Invoice(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
...
@Entity @Table(name = "webhook_events", indexes = [Index(columnList = "externalId")])
class WebhookEvent(
    @Id var id: String = cuid(),
...
// Object Storage
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "object_storages", indexes = [Index(columnList = "user_id"), Index(columnList = "status")])
class ObjectStorage(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
    @Column(name = "order_id", unique = true, nullable = false) var orderId: String = "",
    @Column(unique = true) var contaboStorageId: String = "",
    @Enumerated(EnumType.STRING) var status: ObjectStorageStatus = ObjectStorageStatus.PROVISIONING,
...
// Domain
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "domain_handles", indexes = [Index(columnList = "user_id")])
class DomainHandle(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
    ...
    var email: String = "", var gender: String? = null,
...
@Entity @Table(name = "domain_tld_pricings")
...
@Entity @Table(name = "domains", indexes = [
    Index(columnList = "user_id"), Index(columnList = "status"), Index(columnList = "domainName")
])
class Domain(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
    ...
    var autoRenew: Boolean = true, var whoisPrivacy: Boolean = false,
    var registeredAt: Instant? = null, var expiresAt: Instant? = null,
) : BaseEntity()

// ─────────────────────────────────────────────────────────
// Snapshot
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "snapshots", indexes = [
    Index(columnList = "server_id"), Index(columnList = "contaboSnapshotId")
])
class Snapshot(
    @Id var id: String = cuid(),
    @Column(name = "server_id", nullable = false) var serverId: String = "",
...
// Private Network
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "private_networks", indexes = [Index(columnList = "user_id"), Index(columnList = "contaboNetworkId")])
...
@Entity @Table(name = "private_network_assignments", indexes = [
    Index(columnList = "private_network_id"), Index(columnList = "server_id")
])
@Table(uniqueConstraints = [UniqueConstraint(columnNames = ["private_network_id", "server_id"])])
...
// Firewall
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "firewalls", indexes = [Index(columnList = "user_id"), Index(columnList = "contaboFirewallId")])
class Firewall(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
...
@Entity @Table(name = "firewall_rules", indexes = [Index(columnList = "firewall_id")])
class FirewallRule(
    @Id var id: String = cuid(),
...
@Entity @Table(name = "firewall_assignments", indexes = [
    Index(columnList = "firewall_id"), Index(columnList = "server_id")
])
@Table(uniqueConstraints = [UniqueConstraint(columnNames = ["firewall_id", "server_id"])])
...
// VIP
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "vips", indexes = [Index(columnList = "user_id"), Index(columnList = "ip")])
class Vip(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
...
// Secret & Tag
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "secrets", indexes = [Index(columnList = "user_id")])
class Secret(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
...
@Entity @Table(name = "tags", indexes = [Index(columnList = "user_id")])
class Tag(
    @Id var id: String = cuid(),
    @Column(name = "user_id", nullable = false) var userId: String = "",
    @Column(unique = true) var contaboId: Int = 0,
...
// Monitoring
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "server_monitors", indexes = [Index(columnList = "isActive")])
class ServerMonitor(
    @Id var id: String = cuid(),
...
@Entity @Table(name = "server_checks", indexes = [Index(columnList = "monitor_id, checked_at")])
class ServerCheck(
    @Id var id: String = cuid(),
    ...
    var ramUsedMb: Int? = null, var ramTotalMb: Int? = null,
    ...
    var openConnections: Int? = null, var processesCount: Int? = null,
    var errorMessage: String? = null, var checkDurationMs: Int? = null,
    var checkerNode: String = "primary",
)

@Entity @Table(name = "monitor_alerts", indexes = [Index(columnList = "monitor_id, is_resolved, triggered_at")])
class MonitorAlert(
    @Id var id: String = cuid(),
    @Column(name = "monitor_id", nullable = false) var monitorId: String = "",
    @Column(name = "check_id") var checkId: String? = null,
    @Enumerated(EnumType.STRING) var type: MonitorAlertType = MonitorAlertType.DOWN,
    @Enumerated(EnumType.STRING) var severity: MonitorAlertSeverity = MonitorAlertSeverity.CRITICAL,
    var message: String = "",
    @Column(precision = 10, scale = 2) var value: BigDecimal? = null,
    @Column(precision = 10, scale = 2) var threshold: BigDecimal? = null,
    var triggeredAt: Instant = Instant.now(), var resolvedAt: Instant? = null,
    var isResolved: Boolean = false,
    var notifiedEmail: Boolean = false, var notifiedTelegram: Boolean = false,
    var ackAt: Instant? = null, var ackNote: String? = null,
)

@Entity @Table(name = "uptime_dailies", uniqueConstraints = [UniqueConstraint(columnNames = ["monitor_id", "date"])])
class UptimeDaily(
    @Id var id: String = cuid(),
    @Column(name = "monitor_id", nullable = false) var monitorId: String = "",
    @Column(columnDefinition = "DATE") var date: LocalDate = LocalDate.now(),
    var totalChecks: Int = 0, var upChecks: Int = 0, var downChecks: Int = 0, var degradedChecks: Int = 0,
    ...
    var incidentsCount: Int = 0, var totalDowntimeSeconds: Int = 0,
    var firstDownAt: Instant? = null,
)

@Entity @Table(name = "maintenance_windows", indexes = [Index(columnList = "monitor_id")])
class MaintenanceWindow(
    @Id var id: String = cuid(),
    @Column(name = "monitor_id", nullable = false) var monitorId: String = "",
    var title: String = "",
    var startsAt: Instant = Instant.now(), var endsAt: Instant = Instant.now(),
...
// Email Log
// ─────────────────────────────────────────────────────────
@Entity @Table(name = "email_logs", indexes = [
    Index(columnList = "user_id"), Index(columnList = "status"),
    Index(columnList = "templateName"), Index(columnList = "sent_at")
])
class EmailLog(
    @Id var id: String = cuid(),
    ...
    var subject: String = "",
    var language: String = "en",
    var status: String = "", var triggeredBy: String = "",
