...
class AlertEngineService(
    ...
    suspend fun evaluate(monitor: ServerMonitor, check: ServerCheck) {
        ...
        if (check.status == MonitorStatus.DOWN || check.status == MonitorStatus.TIMEOUT) {
            conditions.add(AlertCondition(MonitorAlertType.DOWN, MonitorAlertSeverity.CRITICAL,
                "Servidor sin respuesta — ${check.errorMessage ?: "sin respuesta"}", null, null))
        }

        // 2. Degraded
        if (check.status == MonitorStatus.DEGRADED) {
            conditions.add(AlertCondition(MonitorAlertType.DEGRADED, MonitorAlertSeverity.WARNING,
                "Servidor degradado — HTTP ${check.httpStatusCode ?: "error"}",
                check.httpStatusCode?.let { BigDecimal(it) }, null))
        }

        // 3. High latency
        if (check.pingMs != null && check.pingMs > monitor.alertThresholdPingMs) {
            conditions.add(AlertCondition(MonitorAlertType.HIGH_LATENCY, MonitorAlertSeverity.WARNING,
                "Latencia alta: ${check.pingMs}ms (umbral: ${monitor.alertThresholdPingMs}ms)",
                BigDecimal(check.pingMs), BigDecimal(monitor.alertThresholdPingMs)))
        }
        ...
        if (check.cpuPercent != null) {
            ...
            if (cpu > monitor.alertThresholdCpuPct) {
                conditions.add(AlertCondition(MonitorAlertType.HIGH_CPU,
                    if (cpu > 95) MonitorAlertSeverity.CRITICAL else MonitorAlertSeverity.WARNING,
                    "CPU al ${cpu}% (umbral: ${monitor.alertThresholdCpuPct}%)",
                    BigDecimal(cpu), BigDecimal(monitor.alertThresholdCpuPct)))
            }
        }
        ...
        if (check.ramPercent != null) {
            ...
            if (ram > monitor.alertThresholdRamPct) {
                conditions.add(AlertCondition(MonitorAlertType.HIGH_RAM,
                    if (ram > 95) MonitorAlertSeverity.CRITICAL else MonitorAlertSeverity.WARNING,
                    "RAM al ${ram}% (umbral: ${monitor.alertThresholdRamPct}%)",
                    BigDecimal(ram), BigDecimal(monitor.alertThresholdRamPct)))
            }
        }
        ...
        if (check.diskPercent != null) {
            ...
            if (disk > monitor.alertThresholdDiskPct) {
                conditions.add(AlertCondition(MonitorAlertType.HIGH_DISK,
                    if (disk > 95) MonitorAlertSeverity.CRITICAL else MonitorAlertSeverity.WARNING,
                    "Disco al ${disk}% (umbral: ${monitor.alertThresholdDiskPct}%)",
                    BigDecimal(disk), BigDecimal(monitor.alertThresholdDiskPct)))
            }
        }
        ...
        if (check.sslDaysUntilExpiry != null) {
            ...
            if (days < 0) {
                conditions.add(AlertCondition(MonitorAlertType.SSL_INVALID, MonitorAlertSeverity.CRITICAL,
                    "Certificado SSL vencido", BigDecimal(days), BigDecimal.ZERO))
            } else if (days < 30) {
                conditions.add(AlertCondition(MonitorAlertType.SSL_EXPIRY,
                    if (days < 7) MonitorAlertSeverity.CRITICAL else MonitorAlertSeverity.WARNING,
                    "Certificado SSL vence en $days días", BigDecimal(days), BigDecimal(30)))
            }
        }
    ...
    private suspend fun createAlertIfNew(monitor: ServerMonitor, check: ServerCheck, condition: AlertCondition) {
        val existing = monitorAlertRepo.findByMonitorIdAndIsResolvedOrderByTriggeredAtDesc(
            monitor.id, false
        ).firstOrNull { it.type == condition.type }

        if (existing != null) return // don't duplicate

        val alert = monitorAlertRepo.save(MonitorAlert(
        ...
        sendNotifications(monitor, TelegramNotifyService.AlertPayload(
            type = alert.type, severity = alert.severity, message = alert.message))

        monitorAlertRepo.save(alert.copy(
            notifiedEmail = monitor.notifyEmail != null,
            notifiedTelegram = monitor.notifyTelegramChatId != null,
        ))
    }

    private suspend fun resolveActiveAlerts(monitor: ServerMonitor, check: ServerCheck) {
        val activeAlerts = monitorAlertRepo.findByMonitorIdAndIsResolvedOrderByTriggeredAtDesc(
            monitor.id, false
        ).filter { it.type == MonitorAlertType.DOWN || it.type == MonitorAlertType.DEGRADED }

        if (activeAlerts.isEmpty()) return

        activeAlerts.forEach { alert ->
            monitorAlertRepo.save(alert.copy(isResolved = true, resolvedAt = Instant.now()))
        }

        sendNotifications(monitor, TelegramNotifyService.AlertPayload(
            type = MonitorAlertType.RECOVERY, severity = MonitorAlertSeverity.OK,
    ...
    private suspend fun sendNotifications(
        ...
        if (monitor.notifyEmail != null) {
            ...
            val subject = if (isRecovery)
                "✅ [RECOVERY] $hostname — volvió a línea"
            else
                "🔴 [${alert.severity}] $hostname — ${alert.type.name.lowercase()}"

            try {
                notifications.send(monitor.notifyEmail, subject, buildEmailHtml(hostname, ip, alert))
            } catch (e: Exception) {
    ...
    private fun buildEmailHtml(hostname: String, ip: String, alert: TelegramNotifyService.AlertPayload): String {
        val isRecovery = alert.type == MonitorAlertType.RECOVERY
