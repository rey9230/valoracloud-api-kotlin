...
class AdminService(
    ...
    fun getStats(period: StatsPeriodDto): AdminStatsDto {
        ...
        val serversByStatus = allServers.groupBy { it.status.name }.mapValues { it.value.size.toLong() }
        val allOrders = orderRepo.findAll()
        val totalOrders = allOrders.size.toLong()
        val ordersByStatus = allOrders.groupBy { it.status.name }.mapValues { it.value.size.toLong() }
        val paidOrders = allOrders.filter { it.status == OrderStatus.PAID || it.status == OrderStatus.ACTIVE }
        ...
        val fromInvoices = allInvoices
            .filter { it.paidAt != null }
            .fold(BigDecimal.ZERO) { acc, i -> acc + i.amount }
        val activeSessions = refreshTokenRepo.findAll()
            .count { it.revokedAt == null && it.expiresAt > Instant.now() }
        val unprocessedWebhooks = webhookEventRepo.findAll().count { !it.processed }

        return AdminStatsDto(
            users = UsersStats(totalUsers, deletedUsers, unverifiedUsers.toLong()),
    ...
    fun getRevenueStats(dto: StatsPeriodDto): RevenueStatsDto {
        val days = when (dto.period) { "7d" -> 7; "90d" -> 90; else -> 30 }
    ...
    fun getNewUsersStats(dto: StatsPeriodDto): NewUsersStatsDto {
        val days = when (dto.period) { "7d" -> 7; "90d" -> 90; else -> 30 }
        val since = Instant.now().minusSeconds(days * 86400L)
        val rows = userRepo.findAll()
            .filter { it.deletedAt == null && it.createdAt.isAfter(since) }
            .groupBy { LocalDate.ofInstant(it.createdAt, java.time.ZoneOffset.UTC) }
            .map { (date, users) -> NewUsersSeriesEntry(date.format(DateTimeFormatter.ISO_LOCAL_DATE), count = users.size) }
            .sortedBy { it.date }
        return NewUsersStatsDto(period = dto.period, series = rows)
    }

    fun getServersByRegion(): List<RegionCount> =
        serverRepo.findAll().groupBy { it.region }.map { RegionCount(it.key, it.value.size.toLong()) }

    fun getOrdersByStatus(): List<StatusCount> =
        orderRepo.findAll().groupBy { it.status.name }.map { StatusCount(it.key, it.value.size.toLong()) }
    ...
    fun listUsers(filter: AdminUsersFilterDto): PaginatedResponse<Map<String, Any?>> {
        ...
        var users = userRepo.findAll(pageable)
        ...
        if (filter.emailVerified != null) users = users.filter { it.emailVerified == filter.emailVerified }
        ...
        val data = users.map { u ->
            mapOf<String, Any?>(
                "id" to u.id, "email" to u.email, "firstName" to u.firstName,
                "lastName" to u.lastName, "role" to u.role.name, "status" to u.status.name,
                "emailVerified" to u.emailVerified, "deletedAt" to u.deletedAt,
                "createdAt" to u.createdAt, "updatedAt" to u.updatedAt,
            )
        }
        return PaginatedResponse(data, total, filter.page ?: 1, filter.limit ?: 20, ((total + filter.limit!! - 1) / filter.limit!!).toInt())
    }
    ...
    fun getUserDetail(userId: String): Map<String, Any?> {
        ...
        return mapOf(
            "id" to user.id, "email" to user.email, "firstName" to user.firstName,
            "lastName" to user.lastName, "role" to user.role.name, "status" to user.status.name,
            "emailVerified" to user.emailVerified, "deletedAt" to user.deletedAt,
            "createdAt" to user.createdAt, "updatedAt" to user.updatedAt,
            "servers" to servers.map { s ->
                mapOf("id" to s.id, "hostname" to s.hostname, "ipAddress" to s.ipAddress,
                    "status" to s.status.name, "os" to s.os, "region" to s.region,
                    "contaboInstanceId" to s.contaboInstanceId,
                    "provisionedAt" to s.provisionedAt, "expiresAt" to s.expiresAt, "createdAt" to s.createdAt)
            },
            "orders" to orders.map { o ->
                val plan = o.planId?.let { planRepo.findByIdOrNull(it) }
                mapOf("id" to o.id, "status" to o.status.name, "totalAmount" to o.totalAmount,
                    "billingCycle" to o.billingCycle, "region" to o.region,
                    "stripePaymentId" to o.stripePaymentId, "createdAt" to o.createdAt,
                    "plan" to plan?.let { mapOf("name" to it.name, "slug" to it.slug) })
            },
            "invoices" to invoices.map { i ->
                mapOf("id" to i.id, "amount" to i.amount, "currency" to i.currency,
                    "stripeInvoiceId" to i.stripeInvoiceId, "paidAt" to i.paidAt, "createdAt" to i.createdAt)
            },
        )
    }

    fun getUserSessions(userId: String) = refreshTokenRepo.findByUserIdAndRevokedAtIsNull(userId)
        .filter { it.expiresAt > Instant.now() }
        .map { mapOf("id" to it.id, "createdAt" to it.createdAt, "expiresAt" to it.expiresAt) }

    fun revokeUserSessions(userId: String): Map<String, Int> {
        val user = userRepo.findByIdOrNull(userId) ?: throw NotFoundException("User", userId)
        val tokens = refreshTokenRepo.findByUserIdAndRevokedAtIsNull(userId)
        tokens.forEach { it.revokedAt = Instant.now(); refreshTokenRepo.save(it) }
    ...
    fun updateUserRole(userId: String, dto: UpdateUserRoleDto): Map<String, Any?> {
        ...
        return mapOf("id" to saved.id, "email" to saved.email, "role" to saved.role.name, "status" to saved.status.name, "updatedAt" to saved.updatedAt)
    }
    ...
    fun deleteUser(userId: String): Map<String, Any?> {
        val user = userRepo.findByIdOrNull(userId) ?: throw NotFoundException("User", userId)
        refreshTokenRepo.findByUserIdAndRevokedAtIsNull(userId).forEach { it.revokedAt = Instant.now(); refreshTokenRepo.save(it) }
    ...
    fun forceVerifyEmail(userId: String): Map<String, Any?> {
        ...
        val saved = userRepo.save(user)
        return mapOf("id" to saved.id, "email" to saved.email, "emailVerified" to saved.emailVerified)
    }
    ...
    fun impersonateUser(userId: String): ImpersonateResponse {
        ...
        return ImpersonateResponse(
            accessToken = token,
            impersonating = mapOf("id" to user.id, "email" to user.email, "role" to user.role.name),
        )
    }
    ...
    fun listServers(filter: AdminServersFilterDto): PaginatedResponse<Map<String, Any?>> {
        ...
        val data = servers.map { s ->
            val user = userRepo.findByIdOrNull(s.userId)
            mapOf(
                "id" to s.id, "hostname" to s.hostname, "ipAddress" to s.ipAddress,
                "status" to s.status.name, "os" to s.os, "region" to s.region,
                "contaboInstanceId" to s.contaboInstanceId,
                "provisionedAt" to s.provisionedAt, "expiresAt" to s.expiresAt, "createdAt" to s.createdAt,
                "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            )
        }
        return PaginatedResponse(data, total, filter.page ?: 1, filter.limit ?: 20, ((total + filter.limit!! - 1) / filter.limit!!).toInt())
    }
    ...
    fun getServerDetail(serverId: String): Map<String, Any?> {
        val server = serverRepo.findByIdOrNull(serverId) ?: throw NotFoundException("Server", serverId)
        ...
        return mapOf(
            "id" to server.id, "hostname" to server.hostname, "ipAddress" to server.ipAddress,
            "status" to server.status.name, "os" to server.os, "region" to server.region,
            "contaboInstanceId" to server.contaboInstanceId, "contaboData" to server.contaboData,
            "provisionedAt" to server.provisionedAt, "expiresAt" to server.expiresAt,
            "createdAt" to server.createdAt, "updatedAt" to server.updatedAt,
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            "order" to order?.let {
                mapOf("id" to it.id, "status" to it.status.name, "totalAmount" to it.totalAmount,
                    "billingCycle" to it.billingCycle, "stripePaymentId" to it.stripePaymentId, "createdAt" to it.createdAt,
                    "plan" to plan?.let { p -> mapOf("name" to p.name, "slug" to p.slug, "cpu" to p.cpu, "ram" to p.ram, "disk" to p.disk) })
            },
            "provisioningLogs" to logs.map { mapOf("id" to it.id, "step" to it.step, "status" to it.status, "message" to it.message, "createdAt" to it.createdAt) },
        )
    }
    ...
    fun adminRestartServer(serverId: String): Map<String, Any?> {
        ...
        return mapOf("id" to server.id, "hostname" to server.hostname, "message" to "Server restarting")
    }
    ...
    fun adminSuspendServer(serverId: String): Map<String, Any?> {
        ...
        if (user != null) {
            ...
            try { notifications.sendServiceSuspendedEmail(user.email, server.hostname, deleteDate, user.language, user.id) }
            catch (e: Exception) { logger.error("Failed service-suspended email: ${e.message}") }
        }
        return mapOf("id" to saved.id, "hostname" to saved.hostname, "status" to saved.status.name)
    }
    ...
    fun adminUnsuspendServer(serverId: String): Map<String, Any?> {
        val server = serverRepo.findByIdOrNull(serverId) ?: throw NotFoundException("Server", serverId)
        if (server.status != ServerStatus.SUSPENDED) throw BadRequestException("Server is not suspended")
        contabo.startInstance(server.contaboInstanceId.toLong())
        server.status = ServerStatus.RUNNING
        val saved = serverRepo.save(server)
        return mapOf("id" to saved.id, "hostname" to saved.hostname, "status" to saved.status.name)
    ...
    fun adminForceServerStatus(serverId: String, status: String): Map<String, Any?> {
        if (status !in listOf("RUNNING", "STOPPED", "ERROR")) throw BadRequestException("Status must be RUNNING, STOPPED, or ERROR")
        val server = serverRepo.findByIdOrNull(serverId) ?: throw NotFoundException("Server", serverId)
    ...
    fun adminReprovisionServer(serverId: String): Map<String, String> {
        ...
        if (server.rootPassword.isBlank()) throw BadRequestException("Server has no stored credentials")
    ...
    fun listOrders(filter: AdminOrdersFilterDto): PaginatedResponse<Map<String, Any?>> {
        ...
        val data = paged.map { o ->
            val user = userRepo.findByIdOrNull(o.userId)
            val plan = o.planId?.let { planRepo.findByIdOrNull(it) }
            mapOf(
                "id" to o.id, "status" to o.status.name, "totalAmount" to o.totalAmount,
                "billingCycle" to o.billingCycle, "region" to o.region,
                "paymentMethod" to o.paymentMethod.name, "serviceType" to o.serviceType.name,
                "stripePaymentId" to o.stripePaymentId, "createdAt" to o.createdAt,
                "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
                "plan" to plan?.let { mapOf("name" to it.name, "slug" to it.slug) },
            )
        }
        return PaginatedResponse(data, total, filter.page ?: 1, filter.limit ?: 20, ((total + (filter.limit ?: 20) - 1) / (filter.limit ?: 20)).toInt())
    }
    ...
    fun getOrderDetail(orderId: String): Map<String, Any?> {
        ...
        return mapOf(
            "id" to order.id, "status" to order.status.name, "totalAmount" to order.totalAmount,
            "billingCycle" to order.billingCycle, "region" to order.region, "os" to order.os,
            "paymentMethod" to order.paymentMethod.name, "stripePaymentId" to order.stripePaymentId,
            "createdAt" to order.createdAt, "updatedAt" to order.updatedAt,
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            "plan" to plan,
            "server" to server?.let { mapOf("id" to it.id, "hostname" to it.hostname, "ipAddress" to it.ipAddress, "status" to it.status.name, "os" to it.os, "region" to it.region, "contaboInstanceId" to it.contaboInstanceId, "provisionedAt" to it.provisionedAt, "expiresAt" to it.expiresAt) },
            "invoices" to invoices.map { mapOf("id" to it.id, "amount" to it.amount, "currency" to it.currency, "stripeInvoiceId" to it.stripeInvoiceId, "paidAt" to it.paidAt, "createdAt" to it.createdAt) },
        )
    }
    ...
    fun updateOrderStatus(orderId: String, dto: UpdateOrderStatusDto): Map<String, Any?> {
        ...
        return mapOf("id" to saved.id, "status" to saved.status.name, "updatedAt" to saved.updatedAt)
    }
    ...
    fun listInvoices(filter: AdminInvoicesFilterDto): PaginatedResponse<Map<String, Any?>> {
        ...
        if (filter.paymentMethod != null) invoices = invoices.filter { it.paymentMethod.name == filter.paymentMethod }
        ...
        val end = minOf(begin + (filter.limit ?: 20), sorted.size)
        val paged = if (begin < sorted.size) sorted.subList(begin, end) else emptyList()
        val data = paged.map { i ->
            val user = userRepo.findByIdOrNull(i.userId)
            mapOf(
                "id" to i.id, "amount" to i.amount, "currency" to i.currency,
                "paymentMethod" to i.paymentMethod.name, "stripeInvoiceId" to i.stripeInvoiceId,
                "paidAt" to i.paidAt, "createdAt" to i.createdAt,
                "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            )
        }
        return PaginatedResponse(data, total, filter.page ?: 1, filter.limit ?: 20, ((total + (filter.limit ?: 20) - 1) / (filter.limit ?: 20)).toInt())
    ...
    fun getInvoiceDetail(invoiceId: String): Map<String, Any?> {
        val invoice = invoiceRepo.findByIdOrNull(invoiceId) ?: throw NotFoundException("Invoice", invoiceId)
        ...
        return mapOf(
            "id" to invoice.id, "amount" to invoice.amount, "currency" to invoice.currency,
            "stripeInvoiceId" to invoice.stripeInvoiceId,
            "paidAt" to invoice.paidAt, "createdAt" to invoice.createdAt,
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            "order" to order?.let { mapOf("id" to it.id, "status" to it.status.name, "billingCycle" to it.billingCycle, "plan" to plan) },
            "server" to server?.let { mapOf("id" to it.id, "hostname" to it.hostname, "ipAddress" to it.ipAddress, "status" to it.status.name) },
        )
    }
    ...
    fun getAllSessions(dto: PaginationDto): PaginatedResponse<Map<String, Any?>> {
        val sessions = refreshTokenRepo.findAll()
        ...
        val data = paged.map { s ->
            val user = userRepo.findByIdOrNull(s.userId)
            mapOf("id" to s.id, "createdAt" to s.createdAt, "expiresAt" to s.expiresAt,
                "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "role" to it.role.name) })
        }
        return PaginatedResponse(data, total, dto.page, dto.limit, ((total + dto.limit - 1) / dto.limit).toInt())
    }
    ...
    fun listPlans(filter: AdminPlansFilterDto): PaginatedResponse<Plan> {
        ...
        if (filter.productType != null) plans = plans.filter { it.productType.name == filter.productType }
        if (!filter.search.isNullOrBlank()) {
            val q = filter.search.lowercase()
            plans = plans.filter { it.name.lowercase().contains(q) || it.slug.lowercase().contains(q) || it.contaboPlanId.lowercase().contains(q) }
        }
        ...
        val end = minOf(begin + (filter.limit ?: 20), sorted.size)
        val paged = if (begin < sorted.size) sorted.subList(begin, end) else emptyList()
        return PaginatedResponse(paged, total, filter.page ?: 1, filter.limit ?: 20, ((total + (filter.limit ?: 20) - 1) / (filter.limit ?: 20)).toInt())
    }

    fun getPlanDetail(planId: String): Plan = planRepo.findByIdOrNull(planId) ?: throw NotFoundException("Plan", planId)

    fun createPlan(dto: CreatePlanDto): Plan = planRepo.save(Plan(
        name = dto.name, slug = dto.slug, productType = ProductType.valueOf(dto.productType),
        description = dto.description, cpu = dto.cpu, ram = dto.ram, disk = dto.disk,
        diskType = dto.diskType ?: "NVMe", bandwidth = dto.bandwidth, portSpeed = dto.portSpeed,
        snapshots = dto.snapshots ?: 0,
        price1Month = dto.price1Month, price6Months = dto.price6Months, price12Months = dto.price12Months,
        setup1Month = dto.setup1Month, setup6Months = dto.setup6Months, setup12Months = dto.setup12Months,
        priceMonthly = dto.price1Month, contaboPlanId = dto.contaboPlanId,
        contaboCostPrice = dto.contaboCostPrice, regions = dto.regions ?: emptyList(),
        availableAddons = dto.availableAddons ?: emptyList(), storageTB = dto.storageTB,
    ...
    fun updatePlan(planId: String, dto: UpdatePlanDto): Plan {
        ...
        dto.name?.let { plan.name = it }; dto.description?.let { plan.description = it }
        dto.cpu?.let { plan.cpu = it }; dto.ram?.let { plan.ram = it }; dto.disk?.let { plan.disk = it }
        dto.price1Month?.let { plan.price1Month = it; plan.priceMonthly = it }
    ...
    fun listTldPricing(): List<Map<String, Any?>> =
        domainTldPricingRepo.findAll().sortedBy { it.tld }.map {
            mapOf("id" to it.id, "tld" to it.tld,
                "registrationPrice" to it.registrationPrice,
                "renewalPrice" to it.renewalPrice,
                "transferPrice" to it.transferPrice,
                "available" to it.available, "createdAt" to it.createdAt, "updatedAt" to it.updatedAt)
        }
    ...
    fun createTldPricing(dto: CreateTldPricingDto): DomainTldPricing {
        ...
        return domainTldPricingRepo.save(DomainTldPricing(
            tld = dto.tld, registrationPrice = dto.registrationPrice,
            renewalPrice = dto.renewalPrice, transferPrice = dto.transferPrice,
            available = dto.available,
        ))
    }

    fun updateTldPricing(id: String, dto: UpdateTldPricingDto): DomainTldPricing {
        val tld = domainTldPricingRepo.findByIdOrNull(id) ?: throw NotFoundException("TldPricing", id)
    ...
    fun deleteTldPricing(id: String): Map<String, String> {
        ...
        if (inUse > 0) throw BadRequestException("TLD is in use by existing domains and cannot be deleted")
    ...
    fun seedTldPricing(): Map<String, Any> {
        ...
        for ((tld, registration, renewal) in default) {
            ...
            if (existing == null) {
                domainTldPricingRepo.save(DomainTldPricing(
                    tld = tld, registrationPrice = BigDecimal(registration),
                    renewalPrice = BigDecimal(renewal), transferPrice = BigDecimal(registration),
                    available = true,
                ))
                created++
            } else skipped++
        }
        return mapOf("message" to "Seed complete", "created" to created, "skipped" to skipped, "total" to default.size)
    }
    ...
    fun getDomains(dto: AdminDomainsFilterDto): Map<String, Any> {
        ...
        val paged = if (begin < sorted.size) sorted.subList(begin, end) else emptyList()
        val data = paged.map { d ->
            val user = userRepo.findByIdOrNull(d.userId)
            val tld = domainTldPricingRepo.findByIdOrNull(d.tldPricingId)
            mapOf(
                "id" to d.id, "domainName" to d.domainName, "status" to d.status.name,
                "autoRenew" to d.autoRenew, "whoisPrivacy" to d.whoisPrivacy,
                "registeredAt" to d.registeredAt, "expiresAt" to d.expiresAt, "createdAt" to d.createdAt,
                "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
                "tldPricing" to tld?.let { mapOf("tld" to it.tld, "renewalPrice" to it.renewalPrice) },
            )
        }
        return mapOf("data" to data, "total" to total, "page" to dto.page, "limit" to dto.limit,
    ...
    fun getDomainDetail(id: String): Map<String, Any?> {
        ...
        return mapOf(
            "id" to domain.id, "domainName" to domain.domainName, "status" to domain.status.name,
            "authCode" to domain.authCode, "autoRenew" to domain.autoRenew,
            "whoisPrivacy" to domain.whoisPrivacy, "nameservers" to domain.nameservers,
            "registeredAt" to domain.registeredAt, "expiresAt" to domain.expiresAt,
            "createdAt" to domain.createdAt, "updatedAt" to domain.updatedAt,
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            "tldPricing" to tld,
            "ownerHandle" to ownerHandle, "adminHandle" to adminHandle,
            "techHandle" to techHandle, "zoneHandle" to zoneHandle,
        )
    }
    ...
    fun updateDomainStatus(id: String, dto: UpdateDomainStatusDto): Domain {
        ...
        if (dto.status == "ACTIVE" && previousStatus != DomainStatus.ACTIVE) {
            ...
            if (user != null) {
                try {
                    notifications.sendDomainRegisteredEmail(user.email, domain.domainName, domain.expiresAt.toString(), user.language, user.id)
                } catch (e: Exception) { logger.warn("Failed domain registered email: ${e.message}") }
            }
        }
        return saved
    }
    ...
    fun getObjectStorages(dto: AdminObjectStoragesFilterDto): Map<String, Any> {
        ...
        val data = paged.map { s ->
            val user = userRepo.findByIdOrNull(s.userId)
            mapOf(
                "id" to s.id, "displayName" to s.displayName, "status" to s.status.name,
                "region" to s.region, "totalPurchasedSpaceTB" to s.totalPurchasedSpaceTB,
                "usedSpaceTB" to s.usedSpaceTB, "s3Endpoint" to s.s3Endpoint,
                "provisionedAt" to s.provisionedAt, "expiresAt" to s.expiresAt, "createdAt" to s.createdAt,
                "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            )
        }
    ...
    fun getObjectStorageDetail(id: String): Map<String, Any?> {
        val storage = objectStorageRepo.findByIdOrNull(id) ?: throw NotFoundException("ObjectStorage", id)
        val user = userRepo.findByIdOrNull(storage.userId)
        return mapOf(
            "id" to storage.id, "displayName" to storage.displayName, "status" to storage.status.name,
            "region" to storage.region, "totalPurchasedSpaceTB" to storage.totalPurchasedSpaceTB,
            "usedSpaceTB" to storage.usedSpaceTB, "autoScaling" to storage.autoScaling,
            "s3Endpoint" to storage.s3Endpoint, "provisionedAt" to storage.provisionedAt,
            "expiresAt" to storage.expiresAt, "createdAt" to storage.createdAt, "updatedAt" to storage.updatedAt,
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
        )
    }
    ...
    fun getFirewalls(dto: AdminFirewallsFilterDto): Map<String, Any> {
        ...
        val data = paged.map { fw ->
            ...
            mapOf(
                "id" to fw.id, "name" to fw.name, "description" to fw.description, "status" to fw.status,
                "contaboFirewallId" to fw.contaboFirewallId, "createdAt" to fw.createdAt,
                "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
                "rulesCount" to rules.size, "assignmentsCount" to assignments.size,
            )
        }
        return mapOf("data" to data, "total" to total, "page" to dto.page, "limit" to dto.limit,
            "totalPages" to ((total + dto.limit - 1) / dto.limit).toInt())
    ...
    fun getFirewallDetail(id: String): Map<String, Any?> {
        ...
        val assignedServers = assignments.mapNotNull { a ->
        ...
        return mapOf(
            "id" to fw.id, "name" to fw.name, "description" to fw.description, "status" to fw.status,
            "contaboFirewallId" to fw.contaboFirewallId, "createdAt" to fw.createdAt, "updatedAt" to fw.updatedAt,
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            "rules" to rules,
    ...
    fun getSnapshots(dto: AdminSnapshotsFilterDto): Map<String, Any> {
        ...
        val data = paged.map { s ->
            ...
            mapOf(
                "id" to s.id, "name" to s.name, "description" to s.description,
                "contaboSnapshotId" to s.contaboSnapshotId, "createdAt" to s.createdAt,
                "server" to server?.let { mapOf("id" to it.id, "hostname" to it.hostname, "ipAddress" to it.ipAddress) },
    ...
    fun getSnapshotDetail(id: String): Map<String, Any?> {
        ...
        val user = server?.let { userRepo.findByIdOrNull(it.userId) }
        return mapOf(
            "id" to snap.id, "name" to snap.name, "description" to snap.description,
            "contaboSnapshotId" to snap.contaboSnapshotId, "createdAt" to snap.createdAt,
            "server" to server?.let { mapOf("id" to it.id, "hostname" to it.hostname, "ipAddress" to it.ipAddress, "region" to it.region, "contaboInstanceId" to it.contaboInstanceId) },
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
        )
    }
    ...
    fun getPrivateNetworks(dto: AdminPrivateNetworksFilterDto): Map<String, Any> {
        ...
        val end = minOf(begin + dto.limit, sorted.size)
        val paged = if (begin < sorted.size) sorted.subList(begin, end) else emptyList()
        val data = paged.map { n ->
            val user = userRepo.findByIdOrNull(n.userId)
            val assignments = privateNetworkAssignmentRepo.findByPrivateNetworkId(n.id)
            mapOf(
                "id" to n.id, "name" to n.name, "description" to n.description,
                "region" to n.region, "dataCenter" to n.dataCenter, "cidr" to n.cidr,
                "contaboNetworkId" to n.contaboNetworkId, "createdAt" to n.createdAt,
                "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
    ...
    fun getPrivateNetworkDetail(id: String): Map<String, Any?> {
        val net = privateNetworkRepo.findByIdOrNull(id) ?: throw NotFoundException("PrivateNetwork", id)
        ...
        return mapOf(
            "id" to net.id, "name" to net.name, "description" to net.description,
            "region" to net.region, "dataCenter" to net.dataCenter, "cidr" to net.cidr,
            "contaboNetworkId" to net.contaboNetworkId, "createdAt" to net.createdAt, "updatedAt" to net.updatedAt,
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
            "assignments" to assignedServers,
        )
    }
    ...
    fun getVips(dto: AdminVipsFilterDto): Map<String, Any> {
        ...
        val end = minOf(begin + dto.limit, sorted.size)
        val paged = if (begin < sorted.size) sorted.subList(begin, end) else emptyList()
        val data = paged.map { v ->
            val user = userRepo.findByIdOrNull(v.userId)
            mapOf(
                "id" to v.id, "ip" to v.ip, "resourceId" to v.resourceId, "resourceType" to v.resourceType,
                "ipVersion" to v.ipVersion, "type" to v.type, "dataCenter" to v.dataCenter, "region" to v.region,
    ...
    fun getVipDetail(id: String): Map<String, Any?> {
        ...
        return mapOf(
            "id" to vip.id, "ip" to vip.ip, "resourceId" to vip.resourceId, "resourceType" to vip.resourceType,
            "ipVersion" to vip.ipVersion, "type" to vip.type, "dataCenter" to vip.dataCenter, "region" to vip.region,
            "createdAt" to vip.createdAt, "updatedAt" to vip.updatedAt,
            "user" to user?.let { mapOf("id" to it.id, "email" to it.email, "firstName" to it.firstName, "lastName" to it.lastName) },
        )
    }
    ...
    fun adminSyncServer(serverId: String): Map<String, Any?> {
        ...
        val newStatus = when (instance.status?.lowercase()) {
        ...
        val saved = serverRepo.save(server)
        return mapOf("id" to saved.id, "hostname" to saved.hostname, "status" to saved.status.name, "contaboData" to saved.contaboData)
    }

    fun adminSyncDomain(domainId: String): Map<String, Any?> {
        val domain = domainRepo.findByIdOrNull(domainId) ?: throw NotFoundException("Domain", domainId)
        val contaboDomain = contabo.getDomain(domain.domainName)
        val newStatus = when (contaboDomain.status?.lowercase()) {
        ...
        val saved = domainRepo.save(domain)
        return mapOf("id" to saved.id, "domainName" to saved.domainName, "status" to saved.status.name, "nameservers" to saved.nameservers)
    }

    fun adminUpdateDomainNameservers(domainId: String, dto: UpdateNameserversDto): Map<String, Any?> {
        val domain = domainRepo.findByIdOrNull(domainId) ?: throw NotFoundException("Domain", domainId)
        contabo.updateDomain(domain.domainName, mapOf("nameservers" to dto.nameservers))
        domain.nameservers = dto.nameservers.map { mapOf("name" to it.name, "ip" to (it.ip ?: "")) }
        val saved = domainRepo.save(domain)
        return mapOf("id" to saved.id, "domainName" to saved.domainName, "nameservers" to saved.nameservers)
    }

    fun adminSyncObjectStorage(storageId: String): Map<String, Any?> {
        val storage = objectStorageRepo.findByIdOrNull(storageId) ?: throw NotFoundException("ObjectStorage", storageId)
        val cs = contabo.getObjectStorage(storage.contaboStorageId)
        val newStatus = when (cs.status?.lowercase()) {
        ...
        return mapOf("id" to saved.id, "displayName" to saved.displayName, "status" to saved.status.name, "totalPurchasedSpaceTB" to saved.totalPurchasedSpaceTB)
    }
    ...
    fun adminReinstallServer(serverId: String, dto: ReinstallServerDto): Map<String, Any?> {
        ...
        if (!uuidRegex.matches(dto.imageId)) {
            val image = contabo.findImageBySlug(dto.imageId)
                ?: throw BadRequestException("No image found for OS slug: ${dto.imageId}")
        ...
        try {
            val secret = contabo.createSecret(ContaboCreateSecretRequest(
                name = "reinstall-${serverId.take(8)}", type = ContaboSecretType.password, value = currentPassword
            ))
            secretId = secret.secretId
        } catch (e: Exception) { logger.warn("Could not create reinstall secret: ${e.message}") }
        ...
        for (i in 0 until 30) {
            ...
            try {
                ...
                if (inst.status == "running") { ready = true; break }
            } catch (_: Exception) { /* poll again */ }
        }

        secretId?.let {
            try { contabo.deleteSecret(it) } catch (e: Exception) { logger.warn("Could not delete secret: ${e.message}") }
        }
        server.status = if (ready) ServerStatus.RUNNING else ServerStatus.STOPPED
        server.os = resolvedImageId
        val saved = serverRepo.save(server)
        return mapOf("id" to saved.id, "hostname" to saved.hostname, "status" to saved.status.name, "os" to saved.os)
    }

    fun adminGetServerLogs(serverId: String) =
        provisioningLogRepo.findByServerIdOrderByCreatedAtAsc(serverId)

    fun adminChangeServerPassword(serverId: String, dto: ChangeServerPasswordDto): Map<String, String> {
        val server = serverRepo.findByIdOrNull(serverId) ?: throw NotFoundException("Server", serverId)
        if (server.ipAddress == null) throw BadRequestException("Server has no IP address yet")
    ...
    fun adminCreateSnapshot(serverId: String, dto: CreateSnapshotDto): Snapshot {
        ...
        if (existingSnaps.size >= 10) throw BadRequestException("Contabo limit: max 10 snapshots per instance")
        val cs = contabo.createSnapshot(server.contaboInstanceId.toLong(),
            ContaboCreateSnapshotRequest(name = dto.name, description = dto.description))
        return snapshotRepo.save(Snapshot(serverId = serverId, contaboSnapshotId = cs.snapshotId, name = cs.name, description = cs.description))
    }

    fun adminUpdateSnapshot(snapshotId: String, dto: UpdateSnapshotDto): Snapshot {
        val snap = snapshotRepo.findByIdOrNull(snapshotId) ?: throw NotFoundException("Snapshot", snapshotId)
        val server = serverRepo.findByIdOrNull(snap.serverId) ?: throw NotFoundException("Server", snap.serverId)
        val cs = contabo.updateSnapshot(server.contaboInstanceId.toLong(), snap.contaboSnapshotId,
    ...
    fun adminRollbackSnapshot(snapshotId: String): Map<String, Any?> {
        ...
        return mapOf("snapshotId" to snap.id, "serverId" to snap.serverId, "serverHostname" to server.hostname, "message" to "Snapshot rollback initiated")
    }
    ...
    fun adminSyncSnapshots(serverId: String): List<Snapshot> {
        ...
        for (cs in csSnaps) {
            ...
            if (existing != null) {
                existing.name = cs.name; existing.description = cs.description
                snapshotRepo.save(existing)
            } else {
                snapshotRepo.save(Snapshot(serverId = serverId, contaboSnapshotId = cs.snapshotId, name = cs.name, description = cs.description))
            }
        }
        // Remove stale
        val dbSnaps = snapshotRepo.findByServerId(serverId)
        dbSnaps.filter { !contaboIds.contains(it.contaboSnapshotId) }.forEach { snapshotRepo.delete(it) }
    ...
    fun adminCreateDomainHandle(userId: String, dto: Map<String, Any>): DomainHandle {
        ...
        return domainHandleRepo.save(DomainHandle(
            userId = userId, contaboHandleId = ch.handleId, handleType = ch.handleType.name,
            firstName = ch.firstName, lastName = ch.lastName,
            organization = ch.organization, email = ch.email,
            gender = ch.gender?.name, birthInfo = null,
            address = mapOf(
    ...
    fun adminUpdateDomainHandle(handleId: String, dto: Map<String, Any>): DomainHandle {
        val handle = domainHandleRepo.findByIdOrNull(handleId) ?: throw NotFoundException("Handle", handleId)
    ...
    fun adminGetStorageCredentials(storageId: String): Map<String, Any?> {
        ...
        return mapOf(
            "accessKey" to creds.accessKey, "secretKey" to creds.secretKey,
            "endpoint" to creds.s3Url, "region" to creds.region,
    ...
    fun adminUpgradeStorage(storageId: String, dto: UpgradeStorageDto): ObjectStorage {
        val storage = objectStorageRepo.findByIdOrNull(storageId) ?: throw NotFoundException("ObjectStorage", storageId)
        contabo.upgradeObjectStorage(storage.contaboStorageId, ContaboUpgradeObjectStorageRequest(
            totalPurchasedSpaceTB = dto.totalPurchasedSpaceTB,
            autoScaling = dto.autoScaling?.let {
                ContaboAutoScaling(
                    state = try { AutoScalingState.valueOf((it["state"] as? String)?.uppercase() ?: "ENABLED") } catch (_: Exception) { AutoScalingState.ENABLED },
                    sizeLimitTB = (it["sizeLimitTB"] as? Number)?.toDouble() ?: dto.totalPurchasedSpaceTB ?: 1.0
                )
            }
        ))
    ...
    fun adminCreateFirewall(userId: String, dto: Map<String, Any>): Firewall {
        ...
        val req = ContaboCreateFirewallRequest(
            name = name,
        ...
        return firewallRepo.save(Firewall(
            userId = userId, contaboFirewallId = cf.firewallId,
            name = dto["name"] as? String ?: "", description = dto["description"] as? String,
        ))
    }

    fun adminUpdateFirewall(firewallId: String, dto: Map<String, Any>): Firewall {
        val fw = firewallRepo.findByIdOrNull(firewallId) ?: throw NotFoundException("Firewall", firewallId)
        contabo.updateFirewall(fw.contaboFirewallId, ContaboUpdateFirewallRequest(name = dto["name"] as? String))
    ...
    fun adminUpdateFirewallRules(firewallId: String, dto: Map<String, Any>): Firewall {
        val fw = firewallRepo.findByIdOrNull(firewallId) ?: throw NotFoundException("Firewall", firewallId)
        contabo.updateFirewallRules(fw.contaboFirewallId,
            ContaboUpdateFirewallRulesRequest(rules = ContaboFirewallRules(inbound = (dto["inbound"] as? List<*>)?.map {
                @Suppress("UNCHECKED_CAST") val r = it as Map<String, Any>
                ContaboFirewallRule(
                    protocol = try { FirewallProtocol.valueOf((r["protocol"] as? String)?.uppercase() ?: "TCP") } catch (_: Exception) { FirewallProtocol.TCP },
                    port = (r["port"] as? Number)?.toInt(),
                    portRange = r["portRange"] as? String,
                    sourceIp = r["sourceIp"] as? String,
                    sourceNet = r["sourceNet"] as? String,
                    action = try { FirewallAction.valueOf((r["action"] as? String)?.uppercase() ?: "ALLOW") } catch (_: Exception) { FirewallAction.ALLOW },
                )
            })))
        ...
        (dto["inbound"] as? List<Map<String, Any>>)?.forEach { r ->
            firewallRuleRepo.save(FirewallRule(
                firewallId = firewallId, protocol = r["protocol"] as? String ?: "tcp",
                port = (r["port"] as? Number)?.toInt(),
                portRange = r["portRange"] as? String,
                sourceIp = r["sourceIp"] as? String,
                sourceNet = r["sourceNet"] as? String,
    ...
    fun adminDeleteFirewall(firewallId: String): Map<String, String> {
        ...
        firewallRuleRepo.findByFirewallId(firewallId).forEach { firewallRuleRepo.delete(it) }
        firewallAssignmentRepo.findByFirewallId(firewallId).forEach { firewallAssignmentRepo.delete(it) }
    ...
    fun adminAssignFirewallToServer(firewallId: String, serverId: String): FirewallAssignment {
        ...
        return firewallAssignmentRepo.save(FirewallAssignment(firewallId = firewallId, serverId = serverId))
    }

    fun adminUnassignFirewallFromServer(firewallId: String, serverId: String): Map<String, String> {
        val fw = firewallRepo.findByIdOrNull(firewallId) ?: throw NotFoundException("Firewall", firewallId)
        val server = serverRepo.findByIdOrNull(serverId) ?: throw NotFoundException("Server", serverId)
        contabo.unassignInstanceFromFirewall(fw.contaboFirewallId, server.contaboInstanceId.toLong())
        firewallAssignmentRepo.findByFirewallId(firewallId).filter { it.serverId == serverId }
    ...
    fun adminCreatePrivateNetwork(userId: String, dto: Map<String, Any>): PrivateNetwork {
        val req = ContaboCreatePrivateNetworkRequest(
            region = dto["region"] as? String ?: "EU",
            name = dto["name"] as String,
            description = dto["description"] as? String,
        )
        val cn = contabo.createPrivateNetwork(req)
        return privateNetworkRepo.save(PrivateNetwork(
            userId = userId, contaboNetworkId = cn.privateNetworkId.toString(),
            name = cn.name ?: "", description = cn.description,
            region = cn.region ?: "EU", dataCenter = cn.dataCenter, cidr = cn.cidr,
        ))
    }

    fun adminUpdatePrivateNetwork(networkId: String, dto: Map<String, Any>): PrivateNetwork {
        val net = privateNetworkRepo.findByIdOrNull(networkId) ?: throw NotFoundException("PrivateNetwork", networkId)
        contabo.updatePrivateNetwork(net.contaboNetworkId.toLong(),
            ContaboUpdatePrivateNetworkRequest(name = dto["name"] as? String, description = dto["description"] as? String))
    ...
    fun adminDeletePrivateNetwork(networkId: String): Map<String, String> {
        ...
        if (assignments.isNotEmpty()) throw BadRequestException("Cannot delete network with assigned servers. Unassign all first.")
    ...
    fun adminAssignServerToNetwork(networkId: String, serverId: String): PrivateNetworkAssignment {
        val net = privateNetworkRepo.findByIdOrNull(networkId) ?: throw NotFoundException("PrivateNetwork", networkId)
        val server = serverRepo.findByIdOrNull(serverId) ?: throw NotFoundException("Server", serverId)
        contabo.assignInstanceToPrivateNetwork(net.contaboNetworkId.toLong(), server.contaboInstanceId.toLong())
        return privateNetworkAssignmentRepo.save(PrivateNetworkAssignment(privateNetworkId = networkId, serverId = serverId))
    }

    fun adminUnassignServerFromNetwork(networkId: String, serverId: String): Map<String, String> {
        val net = privateNetworkRepo.findByIdOrNull(networkId) ?: throw NotFoundException("PrivateNetwork", networkId)
        val server = serverRepo.findByIdOrNull(serverId) ?: throw NotFoundException("Server", serverId)
        contabo.unassignInstanceFromPrivateNetwork(net.contaboNetworkId.toLong(), server.contaboInstanceId.toLong())
        privateNetworkAssignmentRepo.findByPrivateNetworkId(networkId).filter { it.serverId == serverId }
    ...
    fun adminSyncVips(userId: String): Map<String, Int> {
        ...
        for (v in vips) {
            ...
            if (existing != null) {
                ...
                vipRepo.save(Vip(userId = userId, ip = v.ip, resourceId = v.resourceId?.toString(),
                    resourceType = v.resourceType, ipVersion = v.ipVersion,
                    type = v.type.name, dataCenter = v.dataCenter, region = v.region ?: "EU"))
            }
        }
    ...
    fun adminAssignVipToServer(ip: String, serverId: String): Vip {
        ...
        vip.resourceId = server.id; vip.resourceType = "instance"
    ...
    fun adminUnassignVipFromServer(ip: String, serverId: String): Vip {
        ...
        vip.resourceId = null; vip.resourceType = null
        return vipRepo.save(vip)
    }
    ...
    fun adminListSecrets(userId: String) =
        secretRepo.findByUserId(userId)

    fun adminCreateSecret(userId: String, dto: Map<String, Any>): Secret {
        val cs = contabo.createSecret(ContaboCreateSecretRequest(
            name = dto["name"] as String,
            type = try { ContaboSecretType.valueOf(dto["type"] as String) } catch (_: Exception) { ContaboSecretType.password },
            value = dto["value"] as String,
        ))
        return secretRepo.save(Secret(userId = userId, contaboId = cs.secretId.toInt(), name = cs.name, type = cs.type.name))
    }
    ...
    fun adminSyncSecrets(userId: String): Map<String, Int> {
        ...
        for (cs in secrets) {
            ...
            if (existing != null) {
                existing.name = cs.name; existing.type = cs.type.name
                secretRepo.save(existing)
            } else {
                secretRepo.save(Secret(userId = userId, contaboId = cs.secretId.toInt(), name = cs.name, type = cs.type.name))
            }
        }
    ...
    fun adminListTags(userId: String) =
        tagRepo.findByUserId(userId)

    fun adminCreateTag(userId: String, dto: Map<String, Any>): Tag {
        val color = dto["color"] as? String ?: "#6366f1"
        val ct = contabo.createTag(ContaboCreateTagRequest(
            name = dto["name"] as String,
            color = color,
        ))
        return tagRepo.save(Tag(userId = userId, contaboId = ct.tagId.toInt(), name = ct.name, color = ct.color))
    }
    ...
    fun adminSyncTags(userId: String): Map<String, Int> {
        ...
        for (ct in tags) {
            ...
            if (existing != null) {
                existing.name = ct.name ?: ""; existing.color = ct.color
                tagRepo.save(existing)
            } else {
                tagRepo.save(Tag(userId = userId, contaboId = ct.tagId.toInt(), name = ct.name, color = ct.color))
            }
        }
    ...
    fun adminAssignTag(tagId: String, resourceType: String, resourceId: String): Any {
        val tag = tagRepo.findByIdOrNull(tagId) ?: throw NotFoundException("Tag", tagId)
        return contabo.createTagAssignment(tag.contaboId.toLong(), TagResourceType.valueOf(resourceType), resourceId)
    }

    fun adminUnassignTag(tagId: String, resourceType: String, resourceId: String): Map<String, String> {
        val tag = tagRepo.findByIdOrNull(tagId) ?: throw NotFoundException("Tag", tagId)
        contabo.deleteTagAssignment(tag.contaboId.toLong(), TagResourceType.valueOf(resourceType), resourceId)
    ...
    fun sendIncident(dto: SendIncidentDto): Map<String, Int> {
        var sent = 0
        if (dto.userId != null) {
            val user = userRepo.findByIdOrNull(dto.userId) ?: throw NotFoundException("User", dto.userId)
            notifications.sendIncidentEmail(user.email, dto.region, dto.startedAt, dto.services, dto.reference, user.language, user.id)
            sent = 1
        } else {
            val servers = serverRepo.findAll().filter { it.region == dto.region }.distinctBy { it.userId }
            for (s in servers) {
                try {
                    val user = userRepo.findByIdOrNull(s.userId) ?: continue
                    notifications.sendIncidentEmail(user.email, dto.region, dto.startedAt, dto.services, dto.reference, user.language, user.id)
                    sent++
                } catch (e: Exception) { logger.error("Incident email failed for user ${s.userId}: ${e.message}") }
            }
        }
    ...
    fun sendMaintenance(dto: SendMaintenanceDto): Map<String, Int> {
        ...
        if (dto.userId != null) {
            val user = userRepo.findByIdOrNull(dto.userId) ?: throw NotFoundException("User", dto.userId)
            notifications.sendScheduledMaintenanceEmail(user.email, dto.region, dto.window, dto.localTime, dto.duration, dto.impact, dto.reference, dto.hostname, user.language, user.id)
            sent = 1
        } else {
            val servers = serverRepo.findAll().filter { it.region == dto.region }.distinctBy { it.userId }
            for (s in servers) {
                try {
                    ...
                    notifications.sendScheduledMaintenanceEmail(user.email, dto.region, dto.window, dto.localTime, dto.duration, dto.impact, dto.reference, dto.hostname, user.language, user.id)
                    sent++
                } catch (e: Exception) { logger.error("Maintenance email failed for user ${s.userId}: ${e.message}") }
            }
        }
        return mapOf("sent" to sent)
    }
    ...
    private fun mapToHandleRequest(dto: Map<String, Any>): ContaboCreateDomainHandleRequest {
        ...
        return ContaboCreateDomainHandleRequest(
            handleType = try { HandleType.valueOf((dto["handleType"] as? String)?.uppercase() ?: "PERSON") } catch (_: Exception) { HandleType.PERSON },
            firstName = (dto["firstName"] as? String) ?: "",
            lastName = (dto["lastName"] as? String) ?: "",
            organization = dto["organization"] as? String,
            email = (dto["email"] as? String) ?: "",
            gender = dto["gender"]?.let { try { Gender.valueOf((it as String).uppercase()) } catch (_: Exception) { null } },
            birthInfo = if (dto["birthDate"] != null || dto["birthCity"] != null || dto["birthCountry"] != null) {
                ContaboBirthInfo(
            ...
            address = ContaboHandleAddress(
                street = address?.get("street") ?: address?.get("addressLine1") ?: "",
                streetNumber = address?.get("streetNumber") ?: "",
                zipCode = address?.get("postalCode") ?: address?.get("zipCode") ?: "",
