...
@Repository interface RefreshTokenRepository : JpaRepository<RefreshToken, String> {
...
// ─── Plan ────────────────────────────────────────────────
@Repository interface PlanRepository : JpaRepository<Plan, String> {
    fun findBySlug(slug: String): Plan?
    fun findByProductTypeAndStatus(productType: String, status: String): List<Plan>
    fun findByStatusOrderBySortOrderAsc(status: String): List<Plan>
}

// ─── Order ───────────────────────────────────────────────
@Repository interface OrderRepository : JpaRepository<Order, String> {
...
// ─── Server ──────────────────────────────────────────────
@Repository interface ServerRepository : JpaRepository<Server, String> {
...
@Repository interface ProvisioningLogRepository : JpaRepository<ProvisioningLog, String> {
    fun findByServerIdOrderByCreatedAtAsc(serverId: String): List<ProvisioningLog>
}

// ─── Invoice ─────────────────────────────────────────────
@Repository interface InvoiceRepository : JpaRepository<Invoice, String> {
    fun findByUserIdOrderByCreatedAtDesc(userId: String): List<Invoice>
    fun findByOrderId(orderId: String): List<Invoice>
}

@Repository interface WebhookEventRepository : JpaRepository<WebhookEvent, String> {
    fun findByStripeEventId(stripeEventId: String): WebhookEvent?
    fun findByExternalId(externalId: String): List<WebhookEvent>
}

// ─── Object Storage ──────────────────────────────────────
@Repository interface ObjectStorageRepository : JpaRepository<ObjectStorage, String> {
...
// ─── Domain ──────────────────────────────────────────────
@Repository interface DomainRepository : JpaRepository<Domain, String> {
...
@Repository interface DomainHandleRepository : JpaRepository<DomainHandle, String> {
    fun findByUserId(userId: String): List<DomainHandle>
}

@Repository interface DomainTldPricingRepository : JpaRepository<DomainTldPricing, String> {
    fun findByTld(tld: String): DomainTldPricing?
    fun findByAvailableTrue(): List<DomainTldPricing>
}

// ─── Snapshot ────────────────────────────────────────────
@Repository interface SnapshotRepository : JpaRepository<Snapshot, String> {
    fun findByServerId(serverId: String): List<Snapshot>
    fun findByContaboSnapshotId(contaboSnapshotId: String): Snapshot?
}

// ─── Private Network ─────────────────────────────────────
@Repository interface PrivateNetworkRepository : JpaRepository<PrivateNetwork, String> {
    fun findByUserId(userId: String): List<PrivateNetwork>
    fun findByContaboNetworkId(contaboNetworkId: String): PrivateNetwork?
}

@Repository interface PrivateNetworkAssignmentRepository : JpaRepository<PrivateNetworkAssignment, String> {
    ...
    fun findByPrivateNetworkIdAndServerId(networkId: String, serverId: String): PrivateNetworkAssignment?
}

// ─── Firewall ────────────────────────────────────────────
@Repository interface FirewallRepository : JpaRepository<Firewall, String> {
    fun findByUserId(userId: String): List<Firewall>
    fun findByContaboFirewallId(contaboFirewallId: String): Firewall?
}

@Repository interface FirewallRuleRepository : JpaRepository<FirewallRule, String> {
    fun findByFirewallId(firewallId: String): List<FirewallRule>
}

@Repository interface FirewallAssignmentRepository : JpaRepository<FirewallAssignment, String> {
...
// ─── VIP ─────────────────────────────────────────────────
@Repository interface VipRepository : JpaRepository<Vip, String> {
    fun findByUserId(userId: String): List<Vip>
    fun findByIp(ip: String): Vip?
}

// ─── Secret & Tag ────────────────────────────────────────
@Repository interface SecretRepository : JpaRepository<Secret, String> {
    fun findByUserId(userId: String): List<Secret>
    fun findByContaboId(contaboId: Int): Secret?
}

@Repository interface TagRepository : JpaRepository<Tag, String> {
    fun findByUserId(userId: String): List<Tag>
    fun findByContaboId(contaboId: Int): Tag?
}

// ─── Monitoring ──────────────────────────────────────────
@Repository interface ServerMonitorRepository : JpaRepository<ServerMonitor, String> {
    fun findByServerId(serverId: String): ServerMonitor?
    fun findByIsActiveTrue(): List<ServerMonitor>
    fun findByIsActiveTrueAndCheckIntervalSecondsLessThanEqual(checkIntervalSeconds: Int): List<ServerMonitor>
}

@Repository interface ServerCheckRepository : JpaRepository<ServerCheck, String> {
    fun findByMonitorIdOrderByCheckedAtDesc(monitorId: String): List<ServerCheck>
    fun findByMonitorIdAndCheckedAtBetween(monitorId: String, start: Instant, end: Instant): List<ServerCheck>
}

@Repository interface MonitorAlertRepository : JpaRepository<MonitorAlert, String> {
    fun findByMonitorIdAndIsResolvedOrderByTriggeredAtDesc(monitorId: String, isResolved: Boolean): List<MonitorAlert>
    fun findByMonitorIdOrderByTriggeredAtDesc(monitorId: String): List<MonitorAlert>
    fun findByIsResolvedAndNotifiedEmail(isResolved: Boolean, notifiedEmail: Boolean): List<MonitorAlert>
}

@Repository interface UptimeDailyRepository : JpaRepository<UptimeDaily, String> {
...
@Repository interface MaintenanceWindowRepository : JpaRepository<MaintenanceWindow, String> {
    fun findByMonitorId(monitorId: String): List<MaintenanceWindow>
    fun findByMonitorIdAndStartsAtBeforeAndEndsAtAfter(
        monitorId: String, now: Instant, now2: Instant
    ): List<MaintenanceWindow>
}

// ─── Email Log ───────────────────────────────────────────
@Repository interface EmailLogRepository : JpaRepository<EmailLog, String> {
