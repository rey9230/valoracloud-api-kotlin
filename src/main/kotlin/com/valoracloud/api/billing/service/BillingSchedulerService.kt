 ...
 * Runs every day at 9 AM UTC — sends expiration warnings for servers
 * expiring within the next 7 days (avoids duplicates by only notifying
 * when expiresAt is between 6d23h and 7d from now).
 */
@Service
class BillingSchedulerService(
    ...
    fun sendExpirationWarnings() {
        ...
        val servers = serverRepo.findAll().filter { server ->
