...
class GlobalExceptionHandler {
    ...
    fun handleAppException(ex: AppException, request: HttpServletRequest): ResponseEntity<ErrorResponse> =
        ResponseEntity.status(ex.status).body(
            ErrorResponse(ex.status.value(), ex.message, request.requestURI, Instant.now())
        )
    ...
    fun handleValidation(ex: MethodArgumentNotValidException, request: HttpServletRequest): ResponseEntity<ErrorResponse> {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
            ErrorResponse(400, "Validation failed", request.requestURI, Instant.now(), errors)
        )
    }
    ...
    fun handleAccessDenied(ex: AccessDeniedException, request: HttpServletRequest): ResponseEntity<ErrorResponse> =
        ResponseEntity.status(HttpStatus.FORBIDDEN).body(
            ErrorResponse(403, "Access denied", request.requestURI, Instant.now())
        )
    ...
    fun handleUnknown(ex: Exception, request: HttpServletRequest): ResponseEntity<ErrorResponse> {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
            ErrorResponse(500, "Internal server error", request.requestURI, Instant.now())
        )
    }
